...
void heapSort(vector<T>&a) {
    ...
    for (int i = (n - 1) / 2; i >= 0; i--)
        __shiftDown(a, n, i); // 对n个元素heapify, 每次 shiftDown 第 i 个元素
    for (int i = n - 1; i > 0; i--) {
        swap(a[0], a[i]);
        __shiftDown(a, i, 0); // 只对前 i 个元素 heapify，交换之后，shiftDown 第[0]个元素
    }
}
...
const T& median3(vector<T>&a, int left, int right) {
    ...
    // 把 a[center] 放在 a[right] 前面，最后两个数已经大于 pivot 了，后面不必划分了  下标 right - 2
